...
class ListJobs(WsResource):
    ...
    def render_GET(self, txrequest):
	try:
		jobId = txrequest.args['job'][0]
		spiders = self.root.launcher.processes.values()
		running = [{"id": s.job, "spider": s.spider} for s in spiders if s.job == jobId]
		if len(running) != 0:
			return {"status":"running"}
		finished = [s for s in self.root.launcher.finished if s.job == jobId]
		if len(finished) != 0:
			return {"status":"finished", "id": finished[0].job, "spider": finished[0].spider,
				"start_time": finished[0].start_time.isoformat(' '),
				"end_time": finished[0].end_time.isoformat(' ')}
		return {"status":"pendingOrRemoved"}
	except Exception, e:
		project = txrequest.args['project'][0]
		spiders = self.root.launcher.processes.values()
